...
PPC_NAME##_ConfigPrivilege(uint32_t periph, \
                        ARM_PPC_CORSTONE315_SecAttr sec_attr, \
                        ARM_PPC_CORSTONE315_PrivAttr priv_attr) \
{ \
    enum ppc_corstone315_error_t ret; \
 \
    ret = ppc_corstone315_config_privilege(&PPC_DEV, periph, \
                        (enum ppc_corstone315_sec_attr_t)sec_attr, \
                        (enum ppc_corstone315_priv_attr_t)priv_attr); \
 \
    if( ret != PPC_CORSTONE315_ERR_NONE) { \
...
{ \
    return ppc_corstone315_is_periph_priv_only(&PPC_DEV, periph); \
} \
\
static int32_t \
...
{ \
    enum ppc_corstone315_error_t ret; \
 \
    ret = ppc_corstone315_config_security(&PPC_DEV, periph, \
                                (enum ppc_corstone315_sec_attr_t)sec_attr); \
 \
    if( ret != PPC_CORSTONE315_ERR_NONE) { \
        return ARM_DRIVER_ERROR; \
...
{ \
    return ppc_corstone315_is_periph_secure(&PPC_DEV, periph); \
} \
 \
...
{ \
    enum ppc_corstone315_error_t ret; \
 \
    ret = ppc_corstone315_irq_enable(&PPC_DEV); \
 \
    if( ret != PPC_CORSTONE315_ERR_NONE) { \
        return ARM_DRIVER_ERROR; \
    } \
...
{ \
    ppc_corstone315_irq_disable(&PPC_DEV); \
} \
 \
static void PPC_NAME##_ClearInterrupt(void) \
{ \
    ppc_corstone315_clear_irq(&PPC_DEV); \
} \
 \
static bool PPC_NAME##_InterruptState(void) \
{ \
    return ppc_corstone315_irq_state(&PPC_DEV); \
} \
 \
/* PPC Corstone315 PPC_NAME Driver CMSIS access structure */ \
ARM_DRIVER_PPC_CORSTONE315 Driver_##PPC_NAME = { \
    .GetVersion        = PPC_CORSTONE315_GetVersion, \
...
{ \
 ...
 \
/* PPC Corstone315 PPC_NAME Driver CMSIS access structure */ \
extern ARM_DRIVER_PPC_CORSTONE315 Driver_##PPC_NAME; \
ARM_DRIVER_PPC_CORSTONE315 Driver_##PPC_NAME = { \
    .GetVersion        = PPC_CORSTONE315_GetVersion, \
    .Initialize        = PPC_NAME##_Initialize, \
    .Uninitialize      = PPC_NAME##_Uninitialize, \
