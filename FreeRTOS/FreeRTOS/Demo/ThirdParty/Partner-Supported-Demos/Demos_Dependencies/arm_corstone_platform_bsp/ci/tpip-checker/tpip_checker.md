...
merge requests. Its purpose is to look for possible areas in the merge request
files that could affect third party IP (TPIP).
...
If any possible TPIP changes are found then a comment is added to the
MR (Merge Request) detailing what the changes are and in what file (a clickable
link to the file is provided). A warning label is also added to the MR stating
that the MR is not to be merged until all the comments have been checked.
